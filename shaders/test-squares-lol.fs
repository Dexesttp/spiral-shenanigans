...
void main(void) {
    ...
    float radLog = log(radius * 3.0 + 1.0) * 10.0;
    float radData = sin(4.0 * (radLog - radTime));
	float bgOndul = (min(radData + 1.9, 1.0) - 0.9) * 10.0;
    ...
    float latticeBS1F1 = lattice(radLog, angle * branchCount, -radTime * 3.0, 0.0);
    float latticeBS1F2 = lattice(radLog, -angle * branchCount, radTime, M_PI);
    float latticeBS2F1 = lattice(radLog, -angle * branchCount, radTime, - M_PI * 1.0 / 4.0);
    float latticeBS2F2 = lattice(radLog, angle * branchCount, -radTime * 3.0, M_PI * 3.0 / 4.0);
    float latticeB = mix(latticeBS1F1 * latticeBS1F2, latticeBS2F1 * latticeBS2F2, segmentSelection);

    float spiral = sin(radLog - angle * branchCount + 3.0 * radTime);

    float latticeSelection = sharpSin(2.0 * (radLog - radTime) + M_PI * 1.25, 0.1) * 0.5 + 0.5;
    float spiralSelection = sharpSin(1.0 * (radLog - radTime + M_PI * 1.25), 0.1) * 0.5 + 0.5;
	float flareValue = max(0.0, min(radius / 0.1 - 0.2, 1.0));
	gl_FragColor = mix(
        bgColor,
        mix(
            mix(pulseColor, fgColor, latticeA),
            mix(bgColor, fgColor, latticeB),
            latticeSelection * 0.5 + 0.5
        ),
        flareValue * bgOndul
    );
}