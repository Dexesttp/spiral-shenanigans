...
void main(void) {
	...
	float flareValue = max(min(radius
		* (pow(
			cos(radius * branchCount * spiralSlope - rotation * direction * radTime * innerSpeed - rotation * direction * angle * innerSlope) * innerStrength
			+ cos(radTime + rotation * angle * outerSlope) * outerStrength
		, 2.0))
		, 1.0), 0.0);
