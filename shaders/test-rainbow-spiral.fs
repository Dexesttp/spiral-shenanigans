...
void main(void) {
	...
	float piValue = radTime + 3.0 * angle + 20.0 / (radius + 0.5);
	float flareValue = sin(piValue) * 0.5 + 0.5;

	float timedHue = sin(- radTime + angle - 10.0 * radius) * 0.1 + 0.9;
	vec4 newFg = vec4(hsv2rgb(vec3(timedHue, 1.0, 1.0)), 1.0);
	float otherHue = sin(- radTime + 20.0 * radius) * 0.05 + 0.4;
	vec4 newBg = vec4(hsv2rgb(vec3(otherHue, 1.0, 1.0)), 1.0);

	// Mix the spin vector and the flare. This is the final step.
	vec4 spiralColor = mix(newBg, newFg, flareValue);
