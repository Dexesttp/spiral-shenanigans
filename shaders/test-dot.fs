...
void main(void) {
	...
	if(radius > 1.25) {
		...
		float mergedCompute = sin(mod(timespeedup * 0.528 - angle * 50.0, 6.2832)) * 0.5 + 0.5;
		mergeValue = (1.0 + mergedCompute * (radius * 4.0 - 5.0));
	}
	else if(radius > 0.75) {
		float mergedCompute = sin(mod(timespeedup * 0.528 - angle * 50.0, 6.2832)) * 0.5 + 0.5;
		mergeValue = (1.0 - mergedCompute) * (radius * 4.0 - 3.0);
	}
