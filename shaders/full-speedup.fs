...
void main(void) {
	...
	float bgOndul = (sharpSin(
			log(branchCount * C_pattern_spin - radius) * sin(pow(1.6, radius) * C_inter_pattern + radTime * direction * 1.0) * C_rotation_speed_2
			- angle * branchCount * rotation * direction
			- radTime * C_rotation_speed * direction,
		.7 - max(radius - 2.0, .0)) * 0.5 + 0.5)
		* (1.0 - min(1. / (radius * (11.0 / branchCount) * max(resolution.x, resolution.y) / 10. + .1), 1.0));
