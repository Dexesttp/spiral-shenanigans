...
void main(void) {
	...
	float spinValue = offsetSin(
			15.0 * radius
				* (.25 * sin(angle * branchCount - 12. * radius - 2. * dr) + 3.0)
			- 1. * dr
			+ rab
		, 0.3
	);
	...
	vec4 bgResult = mix(bgColor, dimColor, .5 * sin(- radTime + 3. * radius) + .5);
	vec4 spinVector = mix(
		mix(fgColor, pulseColor, .5 * sin(- 2. * radTime + 5. * radius) + .5),
		bgResult,
		spinValue
	);
	...
	gl_FragColor = mix(bgResult, spinVector, flareValue);
}
