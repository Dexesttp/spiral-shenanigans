...
void main(void) {
	...
	float timespeedup = (80.0 + min(0.7*time, 50.0))*time;
	// float timespeedup = 60.0*time;
	float pulseTiming = time * 17.1 / branchCount + radius * 5.0;
	float pulseValue = sin(pulseTiming);
	vec4 pulseColor = vec4(sin(pulseTiming), sin(pulseTiming + radius * 5.0), sin(pulseTiming + radius * 10.0), 1.0);
	vec4 colorVector = mix(vec4(0.5, 0.5, 0.5, 1.0), pulseColor, pulseValue);
