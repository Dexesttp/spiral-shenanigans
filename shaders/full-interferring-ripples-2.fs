...
void main(void) {
	...
	float spinValue = max(min(
		offsetSin(45.0 * radius - 3. * dr + rab, 0.3) * sin(- 2. * dr + radius * 15. - rab)
		+ offsetSin(41.0 * radius - 2. * dr + rab + M_PI_OVER_2, 0.6) * sin(- dr + radius * 27. - rab)
		+ .8
		, 1.), 0.);
	...
	float flareValue2 = max(0., min(1. - log(0.1 + radius * 2.5), 1.));

	// Mix the spin vector and the flare. This is the final step.
	gl_FragColor = mix(bgColor, mix(fgColor, spinVector, flareValue), flareValue2);
}
